# https://kb.synology.com/en-global/DSM/tutorial/What_websites_does_Synology_NAS_connect_to_when_running_services_or_updating_software
...
locals {
  addrs = sort(
    flatten(
      toset(
        [for host in data.dns_a_record_set.hosts : host.addrs]
      )
    )
  )
}
